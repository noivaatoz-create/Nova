    ...
    return (
      <div className="min-h-screen bg-[hsl(220,20%,14%)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[hsl(215,20%,60%)] mb-6">Add some products to proceed to checkout.</p>
          <Link href="/shop">
  ...
  return (
    <div className="min-h-screen bg-[hsl(220,20%,14%)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <Link href="/shop" className="inline-flex items-center gap-2 text-[hsl(215,20%,60%)] hover:text-white mb-8 transition-colors" data-testid="link-back-shop">
          <ArrowLeft className="h-4 w-4" />
          Continue Shopping
        </Link>

        <h1 className="text-3xl font-bold text-white mb-8" data-testid="text-checkout-title">Checkout</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                      className="w-full rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,20%,14%)] px-4 py-2.5 text-white placeholder-[hsl(215,20%,60%)] focus:outline-none focus:ring-1 focus:ring-[hsl(38,92%,50%)] text-sm" placeholder="John Doe" data-testid="input-checkout-name" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium text-white mb-1.5">Email</label>
                    <input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                      className="w-full rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,20%,14%)] px-4 py-2.5 text-white placeholder-[hsl(215,20%,60%)] focus:outline-none focus:ring-1 focus:ring-[hsl(38,92%,50%)] text-sm" placeholder="john@example.com" data-testid="input-checkout-email" />
                  </div>
                  <div className="sm:col-span-2">
                    ...
                    <input type="text" required value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })}
                      className="w-full rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,20%,14%)] px-4 py-2.5 text-white placeholder-[hsl(215,20%,60%)] focus:outline-none focus:ring-1 focus:ring-[hsl(38,92%,50%)] text-sm" placeholder="123 Main St" data-testid="input-checkout-address" />
                  </div>
                  <div>
                    ...
                    <input type="text" required value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })}
                      className="w-full rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,20%,14%)] px-4 py-2.5 text-white placeholder-[hsl(215,20%,60%)] focus:outline-none focus:ring-1 focus:ring-[hsl(38,92%,50%)] text-sm" placeholder="San Francisco" data-testid="input-checkout-city" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" required value={form.state} onChange={(e) => setForm({ ...form, state: e.target.value })}
                        className="w-full rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,20%,14%)] px-4 py-2.5 text-white placeholder-[hsl(215,20%,60%)] focus:outline-none focus:ring-1 focus:ring-[hsl(38,92%,50%)] text-sm" placeholder="CA" data-testid="input-checkout-state" />
                    </div>
                    <div>
                      ...
                      <input type="text" required value={form.zip} onChange={(e) => setForm({ ...form, zip: e.target.value })}
                        className="w-full rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,20%,14%)] px-4 py-2.5 text-white placeholder-[hsl(215,20%,60%)] focus:outline-none focus:ring-1 focus:ring-[hsl(38,92%,50%)] text-sm" placeholder="94102" data-testid="input-checkout-zip" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,18%,18%)] p-6">
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center gap-3 p-4 rounded-md border transition-colors ${
                        paymentMethod === "stripe"
                          ...
                          : "border-[hsl(218,18%,25%)] hover:border-[hsl(38,92%,50%)]/50"
                      }`}
                ...
                <p className="text-[hsl(215,20%,60%)] text-xs mt-4 flex items-center gap-1">
            ...
            <div>
              <div className="rounded-md border border-[hsl(218,18%,25%)] bg-[hsl(220,18%,18%)] p-6 sticky top-24">
                <h2 className="text-lg font-bold text-white mb-6">Order Summary</h2>
                <div className="space-y-4 mb-6">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-3" data-testid={`checkout-item-${item.id}`}>
                      <div className="h-14 w-14 rounded-md overflow-hidden bg-[hsl(218,18%,25%)] flex-shrink-0">
                      ...
                      <p className="text-white text-sm font-medium">${(parseFloat(item.price) * item.quantity).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
                <div className="space-y-2 border-t border-[hsl(218,18%,25%)] pt-4">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white">{shipping === 0 ? <span className="text-emerald-400">Free</span> : `$${shipping.toFixed(2)}`}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-base font-bold pt-2 border-t border-[hsl(218,18%,25%)]">
                    ...
                    <span className="text-white" data-testid="text-checkout-total">${grandTotal.toFixed(2)}</span>
                  </div>
                </div>
                <button
                  type="submit"
                ...
                <div className="flex items-center justify-center gap-2 mt-4 text-xs text-[hsl(215,20%,60%)]">

              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-[hsl(218,35%,17%)]">
                    <th className="p-4 text-[hsl(215,30%,65%)] text-xs font-medium uppercase tracking-wider">Order #</th>
                    <th className="p-4 text-[hsl(215,30%,65%)] text-xs font-medium uppercase tracking-wider">Customer</th>
                    <th className="p-4 text-[hsl(215,30%,65%)] text-xs font-medium uppercase tracking-wider">Items</th>
                    <th className="p-4 text-[hsl(215,30%,65%)] text-xs font-medium uppercase tracking-wider">Total</th>
                    <th className="p-4 text-[hsl(215,30%,65%)] text-xs font-medium uppercase tracking-wider">Payment</th>
                      ...
                      <tr key={order.id} className="hover:bg-white/[0.02] transition-colors" data-testid={`row-order-${order.id}`}>
                        ...
                        <td className="p-4">
                          <div>
                            ...
                            <p className="text-[hsl(215,30%,65%)] text-xs">{order.customerEmail}</p>
                          </div>
                        </td>
                        <td className="p-4 text-[hsl(215,30%,65%)] text-sm">{items.length} item(s)</td>
                        <td className="p-4 text-white text-sm font-medium">${order.total}</td>
                        <td className="p-4">
                          <span className="text-[hsl(215,30%,65%)] text-xs uppercase">{order.paymentProvider || "N/A"}</span>
                        </td>
                        <td className="p-4">
                          <select
                            value={order.status}
                  ...
                  {(!orders || orders.length === 0) && (
                    <tr>
                      <td colSpan={8} className="py-12 text-center">
                        <Package className="h-12 w-12 text-[hsl(215,30%,65%)]/30 mx-auto mb-3" />
                        <p className="text-[hsl(215,30%,65%)]">No orders yet</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {selectedOrder && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[hsl(215,30%,65%)] text-xs mb-1">Customer</p>
                    <p className="text-white text-sm font-medium">{selectedOrder.customerName}</p>
                    <p className="text-[hsl(215,30%,65%)] text-xs">{selectedOrder.customerEmail}</p>
                  </div>
                  <div>
                    <p className="text-[hsl(215,30%,65%)] text-xs mb-1">Shipping Address</p>
                    <p className="text-white text-sm">{selectedOrder.shippingAddress}</p>
                  </div>
                </div>
                <div className="border-t border-[hsl(218,35%,17%)] pt-4">
                  <p className="text-[hsl(215,30%,65%)] text-xs mb-3">Items</p>
                  <div className="space-y-2">
                    {(selectedOrder.items as Array<any>).map((item: any, i: number) => (
                      <div key={i} className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-md overflow-hidden bg-[hsl(218,35%,17%)]">
                ...
                <div className="border-t border-[hsl(218,35%,17%)] pt-4 space-y-1">
                  <div className="flex justify-between text-sm">
                    <span className="text-[hsl(215,30%,65%)]">Subtotal</span>
                    <span className="text-white">${selectedOrder.subtotal}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-[hsl(215,30%,65%)]">Shipping</span>
                    <span className="text-white">${selectedOrder.shipping}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-[hsl(215,30%,65%)]">Tax</span>
                    <span className="text-white">${selectedOrder.tax}</span>
                  </div>
                  <div className="flex justify-between text-base font-bold pt-2 border-t border-[hsl(218,35%,17%)]">
                    <span>Total</span>

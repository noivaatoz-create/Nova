      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        {!isLoading && reviews && reviews.length > 0 && (
          <div className="grid lg:grid-cols-3 gap-12 mb-16">
            <div className="text-center p-8 rounded-md border border-[hsl(218,35%,17%)] bg-[hsl(220,38%,10%)]" data-testid="text-avg-rating">
              <p className="text-6xl font-bold text-white mb-2">{avgRating.toFixed(1)}</p>
              <div className="flex gap-1 justify-center mb-2">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} className={`h-5 w-5 ${i < Math.round(avgRating) ? "text-amber-400" : "text-[hsl(218,35%,17%)]"}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <p className="text-[hsl(215,30%,65%)] text-sm">Based on {reviews.length} reviews</p>
            </div>
            ...
            <div className="lg:col-span-2 flex flex-col justify-center space-y-3">
              {ratingDistribution.map((r) => (
                <div key={r.star} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2.5 rounded-full bg-[hsl(218,35%,17%)]">
                    <div
                      className="h-full rounded-full bg-amber-400 transition-all duration-500"
                      style={{ width: `${r.pct}%` }}
                    />
                  </div>
                  <span className="text-[hsl(215,30%,65%)] text-sm w-12 text-right">{r.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-md border border-[hsl(218,35%,17%)] bg-[hsl(220,38%,10%)] p-6 space-y-3">
                <div className="h-4 bg-[hsl(218,35%,17%)] rounded animate-pulse w-1/3" />

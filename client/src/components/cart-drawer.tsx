        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center" data-testid="text-empty-cart">
            ...
            <Link href="/shop" onClick={() => setIsOpen(false)}>
              <Button className="bg-primary text-foreground no-default-hover-elevate no-default-active-elevate" data-testid="button-continue-shopping">
                Continue Shopping
              </Button>
            </Link>
          </div>
          ...
          <>
            {remaining > 0 && (
              <div className="mt-4 rounded-md bg-primary/10 border border-primary/20 p-3 text-sm" data-testid="text-free-shipping-progress">
            ...
            <div className="flex-1 overflow-y-auto mt-4 space-y-4 pr-1">
              {items.map((item) => (
                <div key={item.id} className="flex gap-4 rounded-md border border-border bg-card p-3" data-testid={`cart-item-${item.id}`}>
                  ...
                  <div className="flex flex-1 flex-col justify-between min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <p className="text-primary font-bold text-sm" data-testid={`text-item-price-${item.id}`}>
            ...
            <div className="mt-auto pt-4 border-t border-border space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-foreground font-semibold" data-testid="text-subtotal">${total.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Shipping</span>
                <span className="text-foreground font-semibold" data-testid="text-shipping">
              ...
              <div className="flex justify-between text-base pt-2 border-t border-border">
                ...
                <span className="text-foreground font-bold" data-testid="text-cart-total">
                  ${(total + (total >= freeShippingThreshold ? 0 : shippingFlatRate)).toFixed(2)}
                </span>
              </div>
              <Link href="/checkout" onClick={() => setIsOpen(false)}>
                <Button
                  className="w-full bg-primary text-foreground font-bold mt-2 no-default-hover-elevate no-default-active-elevate"

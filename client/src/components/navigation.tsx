    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/30 bg-background/80 backdrop-blur-xl" data-testid="nav-header">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2" data-testid="link-home">
          {settings?.showLogoIcon !== "false" && <img src="/images/novaatoz-logo.png" alt="Novaatoz" className={`${logoImgSize} object-contain dark:brightness-0 dark:invert`} />}
            ...
            <SheetContent side="right" className="bg-background border-border w-[280px]">
              <SheetHeader>
                <SheetTitle className="flex items-center gap-2 text-foreground">
                  {settings?.showLogoIcon !== "false" && <img src="/images/novaatoz-logo.png" alt="Novaatoz" className="h-7 w-7 object-contain dark:brightness-0 dark:invert" />}
                  {logoText}
                </SheetTitle>
              </SheetHeader>
